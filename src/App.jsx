...
function App() {
  ...
  const generarFechas = (equipos)=>{
    ...
    if(newEquipos.length%2===0){
      ...
      for(let j=0; j<equipos.length/2;j++){
        ...
        fechaVisual.push([newEquipos[j].nombre, newEquipos[equipos.length-j-1].nombre])
      ...
      //equipos.length = cantidad de fechas
        ...
        for(let j=0; j<equipos.length/2;j++){
          //equipos.length/2 = cantidad de partidos por fecha
          if(newEquipos[j].id === newEquipos[equipos.length-j-1].id) continue
          fecha.push([newEquipos[j].id, newEquipos[equipos.length-j-1].id])
          fechaVisual.push([newEquipos[j].nombre, newEquipos[equipos.length-j-1].nombre])
          //Definimos cada partido con un array que contiene la id de ambos contrincantes
        }
  ...
  return (
    <div className="App">
      <form action="" className='app-form' onSubmit={(e)=>handleCantEquipos(e)}>
        <input id="cant-equipos" type="number" placeholder='ingrese cantidad de equipos' value={cantEquiposInput} onChange={(e)=>{setCantEquiposInput(e.target.value)}}/>
        <button type='submit' id="confirmar-cant-equipos">Aceptar</button>
      </form>
      <button onClick={handleFecha} id = "jugar-fecha">Jugar fecha {nroFecha+1}</button>
      <ListaEquipos listaEquipos = {listaEquipos} setListaEquipos = {setListaEquipos}/>
